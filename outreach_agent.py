...
and sends personalized outreach emails from Elena Reyes to prospects — writing the
result (sent / bounced / replied / SQL) back to each row as it goes.

Second SDR identity, sibling to Vida Monroe. Both share the same contact sheet but
are confined to disjoint row ranges (ROW_RANGE_START/END) so they never email the
same prospect: Vida = rows 2..62533, Elena = rows 62534..end.

On any failure      → emails Chris.
On any reply        → forwards to Elena's mailbox + notifies Manae.
...
SENDER_EMAIL  = "elena@getcprdone.com"
...
# Inert as of the 2026-08-31 cut: effective_daily_cap() returns min(step, BATCH_SIZE), so
# every step here is clamped to 200. Left in place for when the recovery ramp is rebuilt.
...
# contacted, i.e. date_sent >= this date. Elena went live 2026-08-05 (set via repo var).
AGENT_LAUNCH_DATE = (os.environ.get("AGENT_LAUNCH_DATE", "") or "2026-08-05").strip()
...
def send_eod_report(sent_count, bounce_count, bounce_list, replacement_queue, skipped_role, dry_run=False, warmth_breakdown=None, state=None):
    ...
    if not dry_run:
        # Chris only now. Manae was getting this daily from BOTH agents on top of every lead
        # handoff — she asked for one email per lead, not an ops report (Chris, 2026-08-17).
        # Nothing is lost: it all appears in the combined all-SDR EOD email too.
        result = send_email(CHRIS_EMAIL, subject, body)
        if result.get("success"):
            log.info("  → EOD summary sent to Chris")
        else:
            log.error(f"  → EOD summary failed: {result.get('error')}")
    else:
        log.info(f"  [DRY RUN] Would send EOD summary to Chris: {sent_count} sent, {bounce_count} bounced, {reply_count} replies")
...
def update_agent_performance(*, sent_today, replies_7d, sql, customers, replied,
    ...
    # Read-modify-write with retry on 409: multiple SDR agents write this same file (Vida +
    # Joffe report in the same window), so a stale-SHA conflict is expected — re-read and retry.
...
def run_report(dry_run=False):
    ...
    # status). Enforce today <= 7d <= lifetime so a counter can't read as "7d > lifetime".
...
def main():
    ...
    parser.add_argument("--mode", choices=["daily","roster","reply_check","stall_check",
                                           "reconcile","report"], required=True)
